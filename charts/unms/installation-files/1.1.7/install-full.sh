...
install_docker_compose() {
  if ! which docker-compose > /dev/null 2>&1; then
    ...
    curl -fsSL "https://github.com/docker/compose/releases/download/1.9.0/docker-compose-$(uname -s)-$(uname -m)" > ${DOCKER_COMPOSE_INSTALL_PATH}/docker-compose || fail "Failed to download docker-compose"
...
create_data_volumes() {
  ...
  mkdir -p -m u+rwX,g-rwx,o-rwx "${DATA_DIR}/rabbitmq"
}
