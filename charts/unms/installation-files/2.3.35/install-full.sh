...
export MIN_DOCKER_VERSION="20.10.5"
export MIN_DOCKER_COMPOSE_VERSION="1.24.1"
export DEFAULT_DOCKER_COMPOSE_VERSION="1.24.1"
  ...
  if ! version_equal_or_newer "${DOCKER_COMPOSE_VERSION}" "${MIN_DOCKER_COMPOSE_VERSION}" ; then
    if [ "${UNATTENDED}" = "false" ] && [ "${SUPPORTED_DISTRO}" = "true" ] && [ "${EUID}" -eq 0 ]; then
      if confirm "Docker compose version ${DOCKER_COMPOSE_VERSION} is not supported. Would you like to update docker compose automatically?"; then
        case "${DISTRO}" in
          ubuntu|debian)
            if realpath "$(command -v docker-compose)" | grep snap > /dev/null 2>&1; then
              snap refresh docker || fail "Failed to update docker compose from snap for '${DISTRO}'."
            elif realpath "$(command -v docker-compose)" | grep "${DOCKER_COMPOSE_INSTALL_PATH}" > /dev/null 2>&1; then
...
create_docker_compose_file() {
  ...
  if realpath "$(command -v docker-compose)" | grep snap > /dev/null 2>&1 && [ "${TMP_INSTALL_DIR}" = "${SCRIPT_DIR}" ] ; then
